...
import { Strike } from "../target/types/strike";
import {
  Keypair,
  PublicKey,
  SystemProgram,
...
describe("strike", () => {
  ...
  const program = anchor.workspace.Strike as Program<Strike>;
  const admin = provider.wallet as anchor.Wallet;

  let strkMint: PublicKey;
  ...
  describe("initialize", () => {
    it("creates global state and epoch 1", async () => {
      ...
      assert.ok(gs.strkMint.equals(strkMint));
