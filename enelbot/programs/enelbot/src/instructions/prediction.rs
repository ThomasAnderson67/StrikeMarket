...
pub fn commit_handler(
    ...
    require!(
        ...
        EnelbotError::CommitWindowClosed
    );

    // Eligibility: must be staked at tier >= 1, no pending unstake
    require!(miner_state.tier >= 1, EnelbotError::NotEligible);
    require!(
        miner_state.unstake_requested_at == 0,
        EnelbotError::UnstakePending
    );
...
pub fn reveal_handler(
    ...
    require!(
        ...
        EnelbotError::RevealWindowClosed
    );

    require!(!commitment.revealed, EnelbotError::AlreadyRevealed);
    require!(prediction == 1 || prediction == 2, EnelbotError::InvalidPrediction);
