...
pub fn stake_handler(ctx: Context<Stake>, amount: u64) -> Result<()> {
    ...
    require!(new_tier >= 1, EnelbotError::InsufficientStake);
...
pub fn unstake_handler(ctx: Context<Unstake>) -> Result<()> {
    ...
    require!(miner_state.staked_amount > 0, EnelbotError::NothingStaked);
    require!(
        miner_state.unstake_requested_at == 0,
        EnelbotError::AlreadyUnstaking
    );
...
pub fn withdraw_handler(ctx: Context<Withdraw>) -> Result<()> {
    ...
    require!(
        miner_state.unstake_requested_at > 0,
        EnelbotError::NoUnstakePending
    );

    let now = Clock::get()?.unix_timestamp;
    require!(
        now >= miner_state.unstake_requested_at + COOLDOWN_SECONDS,
        EnelbotError::CooldownNotElapsed
    );
